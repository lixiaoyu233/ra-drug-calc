const W = [
  ...
  福: "fú",
  肖: "xiāo"
}, G = Object.keys(m).map((h) => ({
  zh: h,
  pinyin: m[h],
  priority: 99 + h.length,
  length: h.length,
  isSurname: !0
})), T = {
  líng: [
...
class f {
  constructor() {
    this.children = /* @__PURE__ */ new Map(), this.fail = null, this.isEnd = !1, this.pattern = null;
  }
}
class K {
  constructor() {
  ...
  buildTrie(n) {
    for (let g of n) {
      const { zh: i, priority: s } = g;
      let u = this.root;
      for (let z = 0; z < i.length; z++) {
        let o = i.charAt(z);
        u.children.has(o) || u.children.set(o, new f()), u = u.children.get(o);
      }
      u.isEnd = !0, (!u.pattern || s >= u.pattern.priority) && (u.pattern = g);
    }
  }
  // 重新构建树
  reset() {
  ...
  buildFailPointer() {
    let n = [];
    for (let [g, i] of this.root.children)
      i.fail = this.root, n.push(i);
    for (; n.length > 0; ) {
      let g = n.shift();
      for (let [i, s] of g.children) {
        let u = g.fail;
        for (; u !== null && !u.children.has(i); )
          u = u.fail;
        u === null ? s.fail = this.root : s.fail = u.children.get(i), n.push(s);
      }
    }
  }
  // 搜索字符串返回匹配的模式串
  search(n, g = !1) {
    ...
    for (let u = 0; u < n.length; u++) {
      let z = n.charAt(u);
      for (; i !== null && !i.children.has(z); )
        i = i.fail;
      if (i === null)
        i = this.root;
      else {
        i = i.children.get(z), i.isEnd && s.push({
          ...i.pattern,
          index: u - i.pattern.length + 1
        });
        let o = i.fail;
        for (; o !== null && o.isEnd; )
          s.push({
            ...o.pattern,
            index: u - o.pattern.length + 1
          }), o = o.fail;
      }
    }
    return this.filter(s, g);
  }
  // 去除搜索的重叠字符串，按照优先级保留
  filter(n, g = !1) {
    g || (n = n.filter((u) => !u.isSurname));
    const i = [];
    let s = 0;
    for (let u = 0; u < n.length; u++) {
      const { index: z, length: o, priority: y } = n[u];
      z >= s ? (i.push(n[u]), s = z + o) : y > i[i.length - 1].priority && (i[i.length - 1] = n[u], s = z + o);
    }
    return i;
  }
}
...
var Q = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g;
function b(h) {
  return h.replace(Q, "_").length;
}
let t = {};
function f3(h = {}) {
  t = {}, Object.keys(h).sort(
    (i, s) => b(s) - b(i)
  ).forEach((i) => {
    t[i] = h[i];
  });
  const g = Object.keys(t).map((i) => ({
    zh: i,
    pinyin: t[i],
    priority: 999 + b(i),
    length: i.length
  }));
  d.reset(), d.buildTrie([..._, ...g]), d.buildFailPointer();
}
const $ = () => t, A = /^[\uD800-\uDBFF]$/, F = /^[\uDC00-\uDFFF]$/, n3 = (h) => {
  const n = h.charCodeAt(0), g = e[n];
  return g ? g.split(" ")[0] : h;
}, h3 = (h, n, g) => {
  const i = d.search(h, g === "surname");
  let s = 0;
  for (let u = 0; u < h.length; ) {
    const z = i[s];
    if (z && u === z.index) {
      const o = z.pinyin.split(" ");
      let y = 0;
      for (let j = 0; j < z.length; j++)
        A.test(z.zh[j - 1]) && F.test(z.zh[j]) ? n[u + j] = {
          origin: z.zh[j],
          result: "",
          isZh: !0,
          originPinyin: ""
  ...
  return n;
}, q = (h) => h.replace(/(ā|á|ǎ|à)/g, "a").replace(/(ō|ó|ǒ|ò)/g, "o").replace(/(ē|é|ě|è)/g, "e").replace(/(ī|í|ǐ|ì)/g, "i").replace(/(ū|ú|ǔ|ù)/g, "u").replace(/(ǖ|ǘ|ǚ|ǜ)/g, "ü").replace(/(ń|ň|ǹ)/g, "n").replace(/ḿ|m̀/g, "m"), i3 = (h, n = "normal") => {
  const g = h.charCodeAt(0), s = $()[h] || (n === "surname" ? m[h] : "") || e[g] || "";
  return s ? s.split(" ").map((u) => ({
    origin: h,
    result: u,
    isZh: !0,
    originPinyin: u
  ...
  for (let s of n)
    for (let u of W)
      if (s.startsWith(u)) {
        let z = s.slice(u.length);
        B.indexOf(u) !== -1 && H.indexOf(z) !== -1 && (z = U[z]), g.push(u), i.push(z);
        break;
      }
  return {
    final: i.join(" "),
  ...
  const { final: n } = r(h);
  let g = "", i = "", s = "";
  return R.indexOf(q(n)) !== -1 ? (g = n[0], i = n[1], s = n.slice(2)) : (i = n[0] || "", s = n.slice(1) || ""), { head: g, body: i, tail: s };
}, w = (h) => {
  const n = /(ā|ō|ē|ī|ū|ǖ)/, g = /(á|ó|é|í|ú|ǘ|ń|ḿ)/, i = /(ǎ|ǒ|ě|ǐ|ǔ|ǚ|ň)/, s = /(à|ò|è|ì|ù|ǜ|ǹ|m̀)/, u = /(a|o|e|i|u|ü|n)/, z = [];
  return h.split(" ").forEach((y) => {
    n.test(y) ? z.push("1") : g.test(y) ? z.push("2") : i.test(y) ? z.push("3") : s.test(y) ? z.push("4") : u.test(y) ? z.push("0") : z.push("");
  }), z.join(" ");
}, g3 = (h, n) => {
  const g = q(h).split(" "), i = w(n).split(" "), s = [];
  return g.forEach((u, z) => {
    s.push(`${u}${i[z]}`);
  }), s.join(" ");
}, p = (h) => {
  const n = [];
  return h.split(" ").forEach((i) => {
    n.push(i[0]);
  }), n.join(" ");
}, O = (h) => typeof h != "string" ? (console.error(
  "The first param of pinyin is error: " + h + ' is not assignable to type "string".'
), !1) : !0, v = (h, n) => {
  let g = n.nonZh;
  if (g === "removed")
    return h.filter((i) => i.isZh);
  if (g === "consecutive") {
    for (let i = h.length - 2; i >= 0; i--) {
      const s = h[i], u = h[i + 1];
      !s.isZh && !u.isZh && (s.origin += u.origin, s.result += u.result, u.delete = !0);
    }
    return h.filter((i) => !i.delete);
  } else
    return h;
}, N = (h, n) => b(h) === 1 && n.multiple ? i3(h, n.mode) : !1, S = (h, n) => {
  switch (n.pattern) {
    case "pinyin":
      break;
    case "num":
    ...
    case "initial":
      h.forEach((g) => {
        g.result = r(g.result).initial;
      });
      break;
    case "final":
      h.forEach((g) => {
        g.result = r(g.result).final;
      });
      break;
    case "first":
      h.forEach((g) => {
    ...
    case "finalHead":
      h.forEach((g) => {
        g.result = x(g.result).head;
      });
      break;
    case "finalBody":
      h.forEach((g) => {
        g.result = x(g.result).body;
      });
      break;
    case "finalTail":
      h.forEach((g) => {
        g.result = x(g.result).tail;
      });
      break;
  }
}, I = (h, n) => {
  ...
  n.v && h.forEach((g) => {
    g.result = g.result.replace(/ü/g, "v");
  });
}, u3 = (h, n, g) => {
  if (n.multiple && b(g) === 1) {
    let i = "";
  ...
  return n.type === "array" ? h.map((i) => i.result) : n.type === "all" ? h.map((i) => {
    const s = i.isZh ? i.result : "", { initial: u, final: z } = r(s), { head: o, body: y, tail: j } = x(s);
    return {
      origin: i.origin,
      pinyin: s,
      initial: u,
      final: z,
      first: i.isZh ? p(i.result) : "",
      finalHead: o,
      finalBody: y,
      finalTail: j,
      num: Number(w(i.originPinyin)),
      isZh: i.isZh
    };
  }) : h.map((i) => i.result).join(n.separator ?? " ");
}, M = (h) => {
  for (let n = h.length - 2; n >= 0; n--) {
    const g = h[n], i = h[n + 1];
    A.test(g.origin) && F.test(i.origin) && (g.origin += i.origin, g.result += i.result, g.originPinyin = g.result, i.delete = !0, n--);
  }
  return h = h.filter((n) => !n.delete), h;
}, s3 = {
  pattern: "pinyin",
  toneType: "symbol",
  type: "string",
...
function a(h, n) {
  ...
  n.type === "all" && (n.pattern = "pinyin"), n.pattern === "num" && (n.toneType = "none"), n.removeNonZh && (n.nonZh = "removed");
  let i = Array(h.length);
  return i = h3(h, i, n.mode || "normal"), i = M(i), i = v(i, n), N(h, n) && (i = N(h, n)), S(i, n), I(i, n), L(i, n), u3(i, n, h);
}
const z3 = {
  precision: "first",
  continuous: !1,
  ...
  (g == null ? void 0 : g.precision) === "any" && (g.lastPrecision = "any");
  const i = {
    ...z3,
    ...g || {}
  };
  return i.insensitive !== !1 && (h = h.toLowerCase(), n = n.toLowerCase()), i.space === "ignore" && (n = n.replace(/\s/g, "")), (g == null ? void 0 : g.precision) === "any" ? l3(h, n, i) : c3(h, n, i);
}, y3 = (h, n) => {
  let g = 0;
  ...
  for (let s = 0; s < h.length; s++) {
    if (g.space === "ignore" && h[s] === " ") {
      i.push(s);
      continue;
    }
    if (h[s] === n[0]) {
      n = n.slice(1), i.push(s);
      continue;
    }
    const u = a(h[s], {
      toneType: "none",
  ...
  return g.space === "ignore" && (i = i.filter((s) => h[s] !== " ")), i.length ? i : null;
}, c3 = (h, n, g) => {
  const i = h.split(""), s = Array(i.length + 1);
  for (let u = 0; u < s.length; u++)
    s[u] = Array(n.length + 1), s[u][0] = [];
  for (let u = 0; u < s[0].length; u++)
    s[0][u] = [];
  for (let u = 1; u < s.length; u++) {
    if (!g.continuous || g.space == "ignore" && h[u - 1] === " ")
      for (let z = 1; z <= n.length; z++)
        s[u][z - 1] = s[u - 1][z - 1];
    for (let z = 1; z <= n.length; z++)
      if (s[u - 1][z - 1]) {
        if (z !== 1 && !s[u - 1][z - 1].length)
          continue;
        {
          const o = a(h[u - 1], {
            type: "array",
          ...
          if (h[u - 1] === n[z - 1]) {
            const l = [...s[u - 1][z - 1], u - 1];
            if ((!s[u][z] || l.length > s[u][z].length) && (s[u][z] = l), z === n.length)
              return s[u][z];
          }
          if (n.length - z <= o3 && o.some((c) => g.lastPrecision === "any" ? c.includes(n.slice(z - 1, n.length)) : g.lastPrecision === "start" ? c.startsWith(n.slice(z - 1, n.length)) : g.lastPrecision === "first" ? c[0] === n.slice(z - 1, n.length) : g.lastPrecision === "every" ? c === n.slice(z - 1, n.length) : !1))
            return [...s[u - 1][z - 1], u - 1];
          const y = g.precision;
          if (y === "start" && o.forEach((l) => {
            let c = z;
            const k = [...s[u - 1][z - 1], u - 1];
            for (; c <= n.length && l.startsWith(n.slice(z - 1, c)); )
              (!s[u][c] || k.length > s[u][c].length) && (s[u][c] = k), c++;
          }), y === "first" && o.some((l) => l[0] === n[z - 1])) {
            const l = [...s[u - 1][z - 1], u - 1];
            (!s[u][z] || l.length > s[u][z].length) && (s[u][z] = l);
          }
          const j = o.find(
            (l) => l === n.slice(z - 1, z - 1 + l.length)
          );
          if (j) {
            const l = [...s[u - 1][z - 1], u - 1], c = z - 1 + j.length;
            (!s[u][c] || l.length > s[u][c].length) && (s[u][c] = l);
          }
        }
      } else
        continue;
  ...
  return a(h, {
    ...
    if (u.isZh) {
      const z = g.resultClass, o = g.chineseClass, y = g.pinyinClass;
      return `<span class="${z}"><ruby><span class="${o}">${u.origin}</span><rp>(</rp><rt class="${y}">${u.pinyin}</rt><rp>)</rp></ruby></span>`;
    } else
      return g.wrapNonChinese ? `<span class="${g.nonChineseClass}">${u.origin}</span>` : u.origin;
  }).join("");
}, d3 = {
  pattern: "pinyin",
  toneType: "symbol",
...
function a3(h, n = d3) {
  ...
  if (h === "")
    return [];
  n.type === "all" && (n.pattern = "pinyin"), n.pattern === "num" && (n.toneType = "none");
  let i = t3(h);
  i = M(i), i = v(i, n);
  let s = b3(i);
  return s.forEach((z) => {
...
const t3 = (h) => h.split("").map((n) => {
  const g = n.charCodeAt(0), i = e[g] || "";
  return {
    origin: n,
    result: i,
    isZh: !!i,
...
}), b3 = (h) => h.map((n) => n.isZh ? n.result.split(" ").map((g) => ({
  origin: n.origin,
  result: g,
  isZh: !0,
  originPinyin: g
})) : [n]), x3 = (h, n) => n.type === "array" ? Array.from(new Set(h.map((g) => g.result))) : n.type === "all" ? h.map((g) => {
  const i = g.isZh ? g.result : "", { initial: s, final: u } = r(i), { head: z, body: o, tail: y } = x(i);
  return {
    origin: g.origin,
    pinyin: i,
    initial: s,
    final: u,
    first: g.isZh ? p(g.result) : "",
    finalHead: z,
    finalBody: o,
    finalTail: y,
    num: Number(w(g.originPinyin)),
    isZh: g.isZh
  };
}) : Array.from(new Set(h.map((g) => g.result))).join(" "), r3 = {
  separator: " ",
  format: "numToSymbol"
}, D = {
  a: ["a", "ā", "á", "ǎ", "à"],
...
function k3(h, n) {
  n = { ...r3, ...n || {} };
  const g = typeof h;
  return typeof h == "string" && (h = h.split(n.separator || " ")), h = h.map((i) => {
    const s = n == null ? void 0 : n.format;
    return s === "numToSymbol" ? q3(i) : s === "symbolToNum" ? w3(i) : s === "toneNone" ? e3(i) : i;
  }), g === "string" ? h.join(n.separator) : h;
}
function q3(h) {
  const n = Number(h[h.length - 1]);
  if (n >= 0 && n <= 4) {
    for (let g in D)
      if (h.includes(g))
        return h.slice(0, h.length - 1).replace(g, D[g][n]);
    return h;
  } else
    return h;
}
